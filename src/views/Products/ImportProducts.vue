...
const onFileChange = (e) => {
    ...
    reader.onload = (evt) => {
        try {
            preview.value = csvImportService.parse(evt.target.result);
        } catch {
...
const startImport = async () => {
    ...
    results.value = await csvImportService.importProducts(
        preview.value,
        (current, total, sku) => {
...
<template>
    <div class="import-page">
        ...
        <div v-if="preview.length" class="preview">
            <h2>Aperçu — {{ preview.length }} produit(s)</h2>
            <table>
                <thead>
                    <tr>
                        <th>Type</th>
        ...
        <div v-if="done" class="results">
            ...
            <table>
                ...
                <tbody>
                    <tr v-for="r in results" :key="r.sku" :class="r.status">
                        <td>{{ r.sku }}</td>
                        <td>{{ r.status === 'success' ? '' : 'NOn' }}</td>
