...
const onFileChange = async (e) => {
  ...
  try {
    ...
    if (!images.length) {
      errorMsg.value =
        "Aucune image trouvée dans le ZIP (formats acceptés : jpg, png, webp, gif).";
    } else {
...
<template>
  <div class="import-page">
    ...
    <p class="subtitle">
      Chargez un fichier <strong>.zip</strong> contenant des images nommées avec
      le SKU du produit.<br />
      Exemple : <code>sk-l.jpg</code>, <code>sk-m.png</code>
    </p>
    ...
    <div v-if="done" class="results">
      <h2>Résultats</h2>
      <p>
        {{ successCount() }} uploadée(s) &nbsp; {{ errorCount() }} erreur(s)
      </p>
      <table>
        <thead>
          <tr>
        ...
        <tbody>
          <tr v-for="r in results" :key="r.filename" :class="r.status">
            ...
            <td>
              {{ r.status === "success" ? `Produit #${r.id}` : r.message }}
            </td>
          </tr>
        </tbody>
      </table>
