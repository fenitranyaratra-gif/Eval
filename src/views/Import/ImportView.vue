...
// ── PRODUITS ──
const productErrors = ref(null);

const onProductFile = (e) => {
  ...
  reader.onload = (evt) => {
    ...
    productPreview.value = csvImportService.parse(evt.target.result);
    productErrors.value = csvValidator.validateProducts(
...
const customerErrors = ref(null);
const orderErrors = ref(null);
...
const onFileChange = async (e, type) => {
  ...
  if (type === "customers") {
    ...
    customerErrors.value = csvValidator.validateCustomers(
      customerRows.value,
      headers,
    );
    ...
    orderErrors.value = csvValidator.validateOrders(orderRows.value, headers);
  }
};
const allFilesLoaded = computed(
  () =>
    ...
    customerErrors.value?.length === 0 &&
    productErrors.value?.length === 0 &&
    orderErrors.value?.length === 0,
);
...
const startImport = async () => {
  ...
  if (!adminToken) {
    errorMsg.value = "Token admin manquant — connectez-vous d'abord.";
    return;
  }
  ...
  customerResults.value = await csvCustomerImportService.importCustomers(
    customerRows.value,
    (current, total, email) => {
      progress.value = { current, total, label: email };
    },
  );
  ...
  productResults.value = await csvImportService.importProducts(
    productRows.value,
    (current, total, sku) => {
      progress.value = { current, total, label: sku };
    },
  );
  ...
  orderResults.value = await csvOrderImportService.importOrders(
    ...
    (current, total, client) => {
      progress.value = { current, total, label: client };
    },
  );
...
<template>
  <div class="import-global">
    <h1>Import Global</h1>
    <p class="subtitle">les 3 fichiers CSV</p>
    ...
    <div v-if="step === 0" class="upload-section">
      <div class="file-block" :class="{ loaded: customerFile }">
        ...
        <div class="drop-zone" @click="customerInput.click()">
          ...
          <p>
            {{
              customerFile ? customerFile.name : "Choisir fichier CSV clients"
            }}
          </p>
        </div>
        <div v-if="customerErrors?.length" class="validation-errors">
          <p v-for="(err, i) in customerErrors" :key="i" class="err-line">
            {{ err.row === 0 ? "En-tête" : `Ligne ${err.row}` }} —
            <strong>{{ err.field }}</strong> : {{ err.message }}
          </p>
        </div>
        <p
          v-else-if="customerErrors !== null && customerRows.length"
          class="valid-msg"
        >
          CSV valide
        </p>
        <p v-if="customerRows.length" class="count">
          {{ customerRows.length }} client(s) détecté(s)
        </p>
      </div>
      ...
      <div class="file-block" :class="{ loaded: productFile }">
        ...
        <div class="drop-zone" @click="productInput.click()">
          ...
          <p>
            {{
              productFile ? productFile.name : "Choisir fichier CSV produits"
            }}
          </p>
        </div>
        <div v-if="productErrors?.length" class="validation-errors">
          <p v-for="(err, i) in productErrors" :key="i" class="err-line">
            {{ err.row === 0 ? "En-tête" : `Ligne ${err.row}` }} —
            <strong>{{ err.field }}</strong> : {{ err.message }}
          </p>
        </div>
        <p
          v-else-if="productErrors !== null && productRows.length"
          class="valid-msg"
        >
          CSV valide
        </p>
        <p v-if="productRows.length" class="count">
          {{ productRows.length }} produit(s) détecté(s)
        </p>
      </div>
      ...
      <div class="file-block" :class="{ loaded: orderFile }">
        <div class="file-header">
          ...
          <span
            >Commandes <small>(date, heure, client, achat, status)</small></span
          >
        </div>
        <div class="drop-zone" @click="orderInput.click()">
          ...
          />
          <p>
            {{ orderFile ? orderFile.name : "Choisir fichier CSV commandes" }}
          </p>
        </div>
        <div v-if="orderErrors?.length" class="validation-errors">
          <p v-for="(err, i) in orderErrors" :key="i" class="err-line">
            {{ err.row === 0 ? "En-tête" : `Ligne ${err.row}` }} —
            <strong>{{ err.field }}</strong> : {{ err.message }}
          </p>
        </div>
        <p
          v-else-if="orderErrors !== null && orderRows.length"
          class="valid-msg"
        >
          CSV valide
        </p>
        <p v-if="orderRows.length" class="count">
          {{ orderRows.length }} commande(s) détectée(s)
        </p>
      </div>
    ...
    <div v-if="step >= 1 && step <= 3" class="running-section">
      ...
      <div v-if="step > 1 && customerResults.length" class="partial-result">
        <strong>Clients :</strong> {{ ok(customerResults) }} &nbsp;
        {{ err(customerResults) }}
      </div>
      <div v-if="step > 2 && productResults.length" class="partial-result">
        <strong>Produits :</strong> {{ ok(productResults) }} &nbsp;
        {{ err(productResults) }}
      </div>
    </div>
    ...
    <div v-if="step === 4" class="results-section">
      ...
      <details v-if="err(customerResults) > 0">
        ...
        <table>
          ...
          <tbody>
            <tr
              v-for="r in customerResults.filter((r) => r.status === 'error')"
              :key="r.email"
            >
              <td>{{ r.email }}</td>
              <td>{{ r.message }}</td>
      ...
      <details v-if="err(productResults) > 0">
        ...
        <table>
          ...
          <tbody>
            <tr
              v-for="r in productResults.filter((r) => r.status === 'error')"
              :key="r.sku"
            >
              <td>{{ r.sku }}</td>
              <td>{{ r.message }}</td>
      ...
      <details v-if="err(orderResults) > 0">
        ...
        <table>
          ...
          <tbody>
            <tr
              v-for="r in orderResults.filter((r) => r.status === 'error')"
              :key="r.ref"
            >
              <td>{{ r.ref }}</td>
              <td>{{ r.message }}</td>
      ...
      <button
        class="btn-primary"
        :disabled="productImporting || productErrors?.length > 0"
        @click="startProductImport"
      >
        Lancer l'import
      </button>
    </div>
