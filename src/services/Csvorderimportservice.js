...
const apiWithToken = async (endpoint, method = 'GET', body = null, token = null) => {
    const headers = {
...
export const csvOrderImportService = {
    ...
    async importOrders(rows, adminToken, customerPasswords, onProgress) {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                if (!password) {
                    results.push({ ref: `${row.date} ${row.client}`, status: 'error', message: `Mot de passe manquant pour ${row.client}` });
                    continue;
                }

                const customerToken = await loginCustomer(row.client, password);
                if (!customerToken) {
                    results.push({ ref: `${row.date} ${row.client}`, status: 'error', message: `Login échoué pour ${row.client}` });
                    continue;
                }
                ...
                if (!items.length) {
                    results.push({ ref: `${row.date} ${row.client}`, status: 'error', message: 'Aucun produit valide dans achat' });
                    continue;
                }

                // 3. Vider le panier existant
                try {
                    await apiWithToken('/api/v1/customer/cart/empty', 'DELETE', null, customerToken);
                } catch (e) { /* ignore */ }

                // 4. Ajouter chaque produit au panier
                let cartOk = true;
                for (const item of items) {
                    ...
                    if (!productId) {
                        results.push({ ref: `${row.date} ${row.client}`, status: 'error', message: `Produit SKU "${item.sku}" introuvable` });
                        cartOk = false;
                        break;
                    }

                    const added = await apiWithToken(`/api/v1/customer/cart/add/${productId}`, 'POST', {
                    ...
                    if (!added?.data) {
                        results.push({ ref: `${row.date} ${row.client}`, status: 'error', message: `Ajout panier échoué pour SKU "${item.sku}" : ${added?.message}` });
                        cartOk = false;
                        break;
                    }
                }
                ...
                // 5. Adresse
                const addrResponse = await apiWithToken('/api/v1/customer/checkout/save-address', 'POST', {
                    billing: {
                ...
                if (!addrResponse?.data) {
                    results.push({ ref: `${row.date} ${row.client}`, status: 'error', message: 'Adresse échouée : ' + (addrResponse?.message || '') });
                    continue;
                }

                // 6. Livraison
                const shippingMethods = addrResponse?.data?.rates;
                const shippingCode = shippingMethods?.[1]?.rates?.[0]?.method || 'free_free';
                await apiWithToken('/api/v1/customer/checkout/save-shipping', 'POST', {
                    shipping_method: shippingCode,
                }, customerToken);

                // 7. Paiement
                await apiWithToken('/api/v1/customer/checkout/save-payment', 'POST', {
                    payment: { method: 'cashondelivery' },
                }, customerToken);

                // 8. Placer la commande
                const order = await apiWithToken('/api/v1/customer/checkout/save-order', 'POST', {}, customerToken);
    ...
    // ← UN SEUL forEach, pas deux !
    orderDetails?.data?.items?.forEach(item => {
        invoiceItems[item.id] = item.qty_ordered;
        shipmentItems[item.id] = { 1: item.qty_ordered }; // ← bon format
    });
    ...
    console.log('Shipment:', JSON.stringify(shipment));
                    ...
                    results.push({
                        ref: `${row.date} ${row.client}`,
                        status: 'error',
                        message: 'Commande échouée : ' + (order?.message || JSON.stringify(order))
                    });
                }

            } catch (e) {
                results.push({ ref: `${row.date} ${row.client}`, status: 'error', message: e.message });
            }
        }
