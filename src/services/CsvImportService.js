...
const buildProductPayload = (row, categoryId) => ({
  sku: row.sku,
  name: row.name,
      ...
      try {
        ...
        if (row.Categorie) {
          if (categoryCache.has(row.Categorie)) {
            ...
            categoryId = await categorieService.getOrCreateCategory(
              row.Categorie,
            );
        ...
        if (!created?.data?.id) {
          ...
          if (created?.errors)
            msg = Object.values(created.errors).flat().join(" | ");
        ...
        if (!updated?.data?.id) {
          ...
          if (updated?.errors)
            msg = Object.values(updated.errors).flat().join(" | ");
          results.push({ sku: row.sku, status: "error", message: msg });
          continue;
        }
