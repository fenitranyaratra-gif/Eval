...
export const categorieService = {
    ...
    async getAllCategories() {
        try {
            const response = await adminApi('/api/v1/admin/catalog/categories', 'GET');
    ...
    // Création de catégorie avec les bons champs requis
    ...
    // Récupérer les attributs filtrables disponibles
    const attrsResponse = await adminApi('/api/v1/admin/catalog/attributes?pagination=0', 'GET');
    const attributes = attrsResponse?.data
    ...
    const payload = {
        ...
        parent_id: 1,
        channels: [1],
        attributes: attributes  // ← vrais IDs d'attributs
    };

    console.log('Payload:', JSON.stringify(payload));
    const response = await adminApi('/api/v1/admin/catalog/categories', 'POST', payload);
    ...
    async getOrCreateCategory(categoryName) {
        ...
        try {
            // 1. Récupérer toutes les catégories existantes
            const categories = await this.getAllCategories();
            
            console.log('Catégories existantes:', categories.map(c => ({ id: c.id, name: c.name })));

            // 2. Chercher la catégorie par nom
            ...
            if (existingCategory) {
                console.log(`Catégorie "${categoryName}" trouvée (ID: ${existingCategory.id})`);
